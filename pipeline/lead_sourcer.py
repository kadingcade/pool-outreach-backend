...
Real mode: queries a property data API (BatchData / Attom).
...
# Demo properties with realistic Florida data
DEMO_PROPERTIES = {
    "default": {
        "city": "Wesley Chapel", "state": "FL", "zip_code": "33543",
        "lat": 28.1869, "lng": -82.3468,
        "home_value": 575_000, "lot_sqft": 24_965, "year_built": 2013,
        "listing_agent_name": "Sarah Mitchell", "listing_agency": "CENTURY 21 BILL NYE REALTY",
    },
}
...
async def enrich_address(address: str) -> dict:
    ...
    In demo mode, generates realistic data. In real mode, calls property API.
    """
...
def _demo_enrich(address: str) -> dict:
    ...
    # Parse city/state from address if present
    parts = address.split(",")
    city = city_data[0]
    state = city_data[1]
    ...
    if len(parts) >= 2:
        city_part = parts[-2].strip() if len(parts) >= 2 else city
        if city_part:
            city = city_part
...
async def _real_enrich(address: str) -> dict:
    """Geocode and enrich via Google Maps + BatchData."""
    async with httpx.AsyncClient() as client:
        ...
        return {
            ...
            "lat": loc["lat"],
            "lng": loc["lng"],
            "home_value": 0,      # Would come from BatchData/Attom
            "lot_sqft": 0,
            "year_built": 0,
            "listing_agent_name": "",
            "listing_agency": "",
